...
if(isset($_POST['authcode'])){
	if($_POST['authcode']==$_SESSION['authcode']){	//判断验证码是否正确
		if(isset($_POST['username']) && isset($_POST['password'])){	//判断提交了用户名和密码是否为空
			...
			$result=mysqli_query($conn,$sql);//查询数据库记录是否有值
			if($row=mysqli_fetch_array($result)){			//提取元组
				...
				echo '用户名或密码错误!<a href="/index.php">点击返回</a>';
			}
		}
	}
	else{
		echo '验证码错误!<a href="/index.php">点击返回</a>';
	}
}
?>
